...
function AcquireData() {
  ...
  let save = prompt(
    "Thank you " +
      name +
      ". Please enter the full amount of your current savings. Please only enter numbers."
  );
  if (isNaN(save)) {
    do {
      save = prompt("That is not a number " + name + " please try again.");
    } while (isNaN(save));
  }
...
function incomeEntry() {
  let incomeName = prompt(
    "Please enter a name for your source of income (such as the name of your workplace)."
  );
  let incomeAmount = prompt(
    "Please enter your monthly income from this source. Please only enter numbers."
  );
  if (isNaN(incomeAmount)) {
    do {
      incomeAmount = prompt("That is not a number, please try again.");
  ...
  let incomeRepeat = prompt(
    `Would you like to add another source of income ${nameArray[0].subjectName}? Please enter "y" for yes or "n" for no.`
  );
  if (incomeRepeat !== "y" && incomeRepeat !== "n") {
    do {
      incomeRepeat = prompt(`Please enter "y" for yes or "n" for no.`);
...
function expensesEntry() {
  let expensesName = prompt(
    `Please enter a name for one of your expenses (for example "rent").`
  );
  let expensesAmount = prompt(
    `Please indicate the amount you spend on this expenditure. Please only enter numbers.`
  );
  if (isNaN(expensesAmount)) {
    do {
      expensesAmount = prompt("That is not a number, please try again.");
  ...
  let expensesRepeat = prompt(
    `Would you like to add another expense? Please enter "y" for yes or "n" for no.`
  );
  if (expensesRepeat !== "y" && expensesRepeat !== "n") {
    do {
      expensesRepeat = prompt(`Please enter "y" for yes or "n" for no.`);
    } while (expensesRepeat !== "y" && expensesRepeat !== "n");
  } else if (expensesRepeat === "n") {
    alert(
      `Thank you ${nameArray[0].subjectName}. We now have all the information we need.`
    );
    jsonStorage();
