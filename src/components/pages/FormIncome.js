...
class FormIncome extends React.Component {
    handleSubmit = (event) => {
        ...
        if (isNaN(numberIncome) || numberIncome === "") {
            alert("Please enter only numbers for income (no letters, symbols or blanks)."); 
            return; 
        } 
        if (incomeTitle === "") {
    ...
    render() {
        return(
            <form onSubmit={this.handleSubmit}>
                <p>Please submit all your income with its sources for the month of {lastMonth}. Please click "Submit" after you have entered
                each source of income.  You may enter as many sources as you wish. When you have finished click "Save" Then click "Next"
                to proceed to the next section.</p>
               <article>
                <label className="col-lg-4 col-md-6 col-sm-12">
                  <p>Income Name (place of work etc.):</p>
                  <input
                    type="text"
                    name="incomeName"
                  />
                ...
                <button type="reset" onClick={incomeFunction}>Save</button>
